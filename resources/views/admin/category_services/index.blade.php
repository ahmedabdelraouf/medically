...
@section('title', __('admin.category_services'))
@section('content')
    <div class="row">
        <div class="col-md-12">
            <!--begin::Card-->
            <div class="iq-card">
                <div class="iq-card-header d-flex justify-content-between">
                    <div class="iq-header-title">
                        <h4 class="card-title">
                            {{__('admin.category_services')}}
                        </h4>
                    </div>
                </div>
                <a href="{{route('category-service.create')}}"
                   class="btn btn-text-primary font-weight-bold btn-fixed" data-palcement="top" data-toggle="tooltip"
                   title="{{trans('admin.add_category_service')}}">
                    ...
                    <table class="table table-striped table-bordered mt-4 table-hover text-center datatable-example"
                        ...
                        @foreach($allServices as $index=>$item)
                            <tr>
                                <td>{{$index += 1}}</td>
                                <td>
                                    <img src="{{asset($item->image)}}" style="width: 5rem;height: 5rem"/>
                                </td>
                                <td>{{$item->name}}</td>
                                ...
                                <td class="text-center">
                                    <div class="flex align-items-center list-user-action">
                                        <a class="iq-bg-primary" data-toggle="tooltip" data-placement="top"
                                           ...
                                           href="{{route('category-service.edit',$item->id)}}">
                                            ...
                                            <div class="modal-dialog" role="document">
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body">
                                                        <p class="text-left">
                                                            {{__('admin.category_service_confirm_delete')}}
                                                        </p>
                                                    </div>
                                                    <div class="modal-footer">
                                                        <button type="button" class="btn btn-secondary"
                                                                data-dismiss="modal">{{__('admin.cancel')}}</button>
                                                        {!! Form::open(['method'=>'delete' , 'route'=>['category-service.destroy' , [$item->id]] , 'class'=>'d-inline']) !!}
